...
func init() {
	config.Hosts = beego.AppConfig.String("MongoDbHosts")
	config.Database = beego.AppConfig.String("MongoDbName")
	config.UserName = beego.AppConfig.String("MongoDbUser")
	config.Password = beego.AppConfig.String("MongoDbPwd")
}
...
func Startup(sessionID string) error {
	...
	tracelog.Trace(sessionID, "Startup", "MongoDB : Host[%s]", config.Hosts)
...
// Execute the Config DB literal function.
func ExecuteDb(sessionID string, mongoSession *mgo.Session, collectionName string, dbCall DBCall) error {
	return Execute(sessionID, mongoSession, config.Database, collectionName, dbCall)
}
